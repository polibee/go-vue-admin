...
func GenerateResource(options ResourceOptions) error {
	...
	if err := registerResourceInModule(filepath.Join(moduleDir, "admin", "module.ts"), options.Manifest.ID); err != nil {
		return err
	}
	...
	return registerResourceInApplicationRoutes(rootDir, options.Module)
}
...
func renderBackendResource(manifest ResourceManifest) string {
	...
	fmt.Fprintln(&b, "}")
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "func RegisterRoutes(auth *controllers.AuthController) error {")
	fmt.Fprintln(&b, "\trepository, err := resource.NewMySQLCoreResourceRepository(resource.MySQLCoreResourceOptions{")
...
func registerResourceInBackendModule(path, module, resourceID string) error {
	...
	text := string(content)
	resourceImport := fmt.Sprintf("\t%s \"go-vue-admin-module/%s/resources/%s\"", resourceID, module, resourceID)
	...
	function := "func RegisterRoutes(auth *controllers.AuthController) error {"
	if !strings.Contains(text, function) {
		text += fmt.Sprintf("\n\n%s\n\treturn %s.RegisterRoutes(auth)\n}\n", function, resourceID)
	}
	return os.WriteFile(path, []byte(text), 0o644)
}
...
func registerResourceInApplicationRoutes(rootDir, module string) error {
	...
	call := fmt.Sprintf("\tif err := %s.RegisterRoutes(authController); err != nil {\n\t\tpanic(err)\n\t}", alias)
