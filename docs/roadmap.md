...
Resource Registry、字段、列、标准列表查询（分页、搜索、排序、字段驱动的 select/boolean 筛选、用户状态筛选）、用户批量设置状态、声明式 Custom Action 和系统概览 Custom Page 已完成；users 资源已补齐创建、编辑、详情和删除闭环，metadata-driven 用户/角色表单已接入；通用资源列表/详情页已改为完全按 Manifest Action 权限判断，不再依赖 users/roles 的页面硬编码。
...
字段权限阶段已完成契约、后端裁剪和 RBAC 持久化，当前正在同步生成器、前端页面和真实资源验收。字段策略包括 `visible/readable/writable/sensitive`，默认保持现有资源兼容行为。
