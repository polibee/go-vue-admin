...
Resource Registry、字段、列、标准列表查询（分页、搜索、排序、字段驱动的 select/boolean 筛选、用户状态筛选）、用户批量设置状态、声明式用户 Custom Action 和系统概览 Custom Page 已完成；users 资源已补齐创建、编辑、详情和删除闭环，metadata-driven 用户/角色表单已接入。
...
当前已完成基础渲染能力：`admin:make-resource` 具备确定性全链路骨架、README、冲突预检和 Golden File 测试；权限和菜单渲染仅作为 Resource Generator 内部能力保留，`admin:make-module` 不再作为公开流程。生成专属 discovery 文件已接入后端 Registry、前端 router 和 Admin Shell，资源生成后可自动进入后台面板。生成前完整检查冲突，只允许更新带生成标记的 discovery 文件，不自动执行迁移。
...
Dashboard 已按 Registry 和当前权限动态生成资源入口，顶部资源导航搜索已完成；搜索仅定位资源入口，不执行跨表数据检索。后续再评估文件管理、全局数据搜索、插件 SDK 和 AI 辅助生成。
