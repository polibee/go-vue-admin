...
test('modules and plugins have separate admin pages', async ({ page }) => {
  ...
  await expect(page.locator('[data-slot="card-title"]').filter({ hasText: '示例模块' })).toBeVisible()
  ...
  await expect(page.locator('[data-slot="card-title"]').filter({ hasText: '示例插件' })).toBeVisible()
  ...
  await expect(page.getByRole('link', { name: '示例插件', exact: true })).toBeVisible()
  await page.getByRole('link', { name: '示例插件', exact: true }).click()
  await expect(page).toHaveURL(/\/admin\/example-plugin$/)
  ...
  await expect(page.getByRole('link', { name: '示例插件', exact: true })).toHaveCount(0)
})
