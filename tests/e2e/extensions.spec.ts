...
test('modules register in the admin and builtin plugins can be enabled and disabled', async ({ page }) => {
  ...
  await expect(page.getByRole('tab', { name: '业务模块' })).toHaveAttribute('aria-selected', 'true')
  await expect(page.getByText('示例模块', { exact: true })).toBeVisible()
  await page.getByRole('tab', { name: '平台插件' }).click()
  await expect(page.getByRole('tab', { name: '平台插件' })).toHaveAttribute('aria-selected', 'true')
  await expect(page.getByText('示例插件', { exact: true })).toBeVisible()
  await page.getByRole('tab', { name: '业务模块' }).click()
  await expect(page.getByText('示例模块', { exact: true })).toBeVisible()
  await page.getByRole('link', { name: '查看' }).first().click()
  await expect(page).toHaveURL(/\/admin\/extensions\/example\?kind=module$/)
  ...
  await expect(page).toHaveURL(/\/admin\/extensions\/example-plugin\?kind=plugin$/)
